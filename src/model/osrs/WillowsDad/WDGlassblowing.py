...
class OSRSWDGlassblowing(WillowsDadBot):
    def __init__(self):
        bot_title = "Glassblower"
        ...
        self.delay_min =0.37
        self.delay_max = .67
        self.blowing_tools = [ids.GLASSBLOWING_PIPE]
    ...
    def main_loop(self):
        ...
        while time.time() - self.start_time < self.end_time:
            ...
            try:
                ...
                if len(self.api_m.get_inv_item_indices(ids.MOLTEN_GLASS)) == 0:
                    self.open_bank()
    ...
    def setup(self):
        ...
        self.lanternlens = 0
        self.text_box_lanternlens = self.WILLOWSDAD_IMAGES.joinpath("textboxlanternlens.png")
    ...
    def make_glass(self):
        ...
        item_found = imsearch.search_img_in_rect(self.text_box_lanternlens, self.win.chat)
        search_time = time.time()
        while not item_found:
            item_found = imsearch.search_img_in_rect(self.text_box_lanternlens, self.win.chat)
            if time.time() - search_time > 2:
                self.log_msg("Error finding lens in chat box, stopping.")
                self.stop()
            time.sleep(.1)
        time.sleep(self.random_sleep_length())
        pag.press("space")